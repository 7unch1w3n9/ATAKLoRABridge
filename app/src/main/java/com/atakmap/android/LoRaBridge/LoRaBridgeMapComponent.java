...
public class LoRaBridgeMapComponent extends DropDownMapComponent {
    ...
    public void onCreate(final Context context, Intent intent,
                         final MapView view, Activity activity) {

        context.setTheme(R.style.ATAKPluginTheme);
        super.onCreate(context, intent, view);
        pluginContext = context;

        ddr = new LoRaBridgeDropDownReceiver(
                view, context, activity);

        Log.d(TAG, "registering the plugin filter");
        DocumentedIntentFilter ddFilter = new DocumentedIntentFilter();
        ddFilter.addAction(com.atakmap.android.LoRaBridge.LoRaBridgeDropDownReceiver.SHOW_PLUGIN);
