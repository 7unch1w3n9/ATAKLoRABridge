...
std::string SoapySDR::getRootPath(void)
{
    ...
    return "/mnt/d/SS2025/SoapySDR-master/build-android-armv7/install";
}
...
#else
#   if defined(__ANDROID__)
    // Android(API<28) 无 glob()，改用目录枚举 + 后缀匹配
    DIR* dir = opendir(path.c_str());
    if (dir != nullptr)
    {
        struct dirent* ent;
        const std::string suffix = "so"; // CMake 配置时替换，通常 ".so"
        while ((ent = readdir(dir)) != nullptr)
        {
            ...
            {
                modulePaths.push_back(path + name);
            }
        }
        closedir(dir);
    }
#   else
    ...
    else
    {
        SoapySDR::logf(SOAPY_SDR_ERROR,
            "SoapySDR::listModules(%s) glob(%s) error %d",
            path.c_str(), pattern.c_str(), ret);
    }
